...
inline void unit_stw(IRFunc* func, IRGenerator* gen)
{
	...
	Instruction instr;
	instr.ops.push_back(11); // RR RR12
	instr.ops.push_back(0xFFF4); // Displ FFF8
	instr.ops.push_back(1); // gpr R1

	//auto stack_val = llvm::ConstantInt::get(gen->m_builder->getInt64Ty(), 0x12454, "stack_val");
...
inline void unit_stwu(IRFunc* func, IRGenerator* gen)
{
	...
	Instruction instr;
	instr.ops.push_back(21); // RR RR12
...
{
	Instruction instr;
	instr.ops.push_back(11); // rD R21
	instr.ops.push_back(0x0); // rA
	instr.ops.push_back(0x212); // imm
...
{
	Instruction instr;
	instr.ops.push_back(12); // rD R21
	instr.ops.push_back(0xFFF4); // displ
...
inline void unit_cmpw(IRFunc* func, IRGenerator* gen)
{
	...
	// to test zero
	auto a_val = llvm::ConstantInt::get(gen->m_builder->getInt64Ty(), 12, "stack_val");
	...
	gen->m_builder->CreateStore(extractCRBit(func, 24), func->getRegister("RR", 30));
}