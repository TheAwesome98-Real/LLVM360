...
struct XEXOptionalHeader
{
	...
	{
		if (!reader.Read(&key, sizeof(key))) return false;
		if (!reader.Read(&offset, sizeof(offset))) return false;
		Swap32(&key);
		Swap32(&offset);
		length = 0;
		value = 0;
...
struct XEXResourceInfo
{
	...
	bool Read(ImageByteReaderXEX& reader)
	{
		if (!reader.Read(&name, 8)) return false;
		if (!reader.Read(&address, 4)) return false;
		if (!reader.Read(&size, 4)) return false;
		Swap32(&address);
		Swap32(&size);
		return true;
	}
...
struct XEXExecutionInfo
{
	...
	bool Read(ImageByteReaderXEX& reader)
	{
		if (!reader.Read(this, sizeof(XEXExecutionInfo))) return false;
		Swap32(&media_id);
...
struct XEXTLSInfo
{
	...
	bool Read(ImageByteReaderXEX& reader)
	{
		if (!reader.Read(this, sizeof(XEXTLSInfo))) return false;
		Swap32(&slot_count);
...
struct XEXImportLibraryBlockHeader
{
	...
	bool Read(ImageByteReaderXEX& reader)
	{
		if (!reader.Read(this, sizeof(XEXImportLibraryBlockHeader))) return false;
		Swap32(&count);
...
struct XEXImportLibraryHeader
{
	...
	bool Read(ImageByteReaderXEX& reader)
	{
		if (!reader.Read(this, sizeof(XEXImportLibraryHeader))) return false;
		Swap32(&unknown);
		Swap32(&import_id);
		Swap32(&version.value);
...
struct XEXFileNormalCompressionInfo
{
	...
	bool Read(ImageByteReaderXEX& reader)
	{
		if (!reader.Read(&window_size, sizeof(window_size))) return false;
		if (!reader.Read(&block_size, sizeof(block_size))) return false;
		if (!reader.Read(&block_hash, sizeof(block_hash))) return false;
		Swap32(&window_size);
		Swap32(&block_size);
...
struct XEXEncryptionHeader
{
	...
	bool Read(ImageByteReaderXEX& reader)
	{
		if (!reader.Read(this, sizeof(XEXEncryptionHeader))) return false;
...
struct XEXLoaderInfo
{
	...
	bool Read(ImageByteReaderXEX& reader)
	{
		if (!reader.Read(this, sizeof(XEXLoaderInfo))) return false;
		Swap32(&header_size);
		Swap32(&image_size);
		Swap32(&unklength);
		Swap32(&image_flags);
